...
use crate::config::{FrpServer, ServiceType, SubdomainConfig, Tunnel, TunnelState, TunnelTarget, TunnelWithState};
...
pub fn update_frp_server(
    ...
    let config_store = lock!(state.config_store)?;
    config_store.update_frp_server(server_id, name, server_addr, server_port, token, subdomain_host, is_default)
}
...
pub async fn list_tunnels(state: State<'_, AppState>) -> Result<Vec<TunnelWithState>, String> {
    let (tunnels, servers, instances) = {
        ...
        (config.tunnels.clone(), config.frp_servers.clone(), config.instances.clone())
    };

    let frpc_manager = FrpcManager::new()?;
    let running_tunnels = frpc_manager.get_running_tunnel_ids();

    let result: Vec<TunnelWithState> = tunnels.iter().map(|t| {
        ...
        let target_name = match &t.target {
            TunnelTarget::Instance(id) => {
                instances.iter().find(|i| i.id == *id).map(|i| i.name.clone())
            }
...
fn reload_frpc_if_running(state: &State<'_, AppState>) -> Result<(), String> {
    ...
    let (tunnels, servers, instances, admin_config) = {
        ...
        let admin_config = config.instances.iter()
            .find(|i| i.service_type == ServiceType::Frpc)
            .map(|instance| {
                let user = instance.config.get("admin_user")
                    .and_then(|v| v.as_str())
                    .unwrap_or("admin")
                    .to_string();
                let password = instance.config.get("admin_password")
                    .and_then(|v| v.as_str())
                    .unwrap_or("admin")
                    .to_string();
                FrpcAdminConfig {
        ...
        (config.tunnels.clone(), config.frp_servers.clone(), config.instances.clone(), admin_config)
    };
...
pub fn create_tunnel(
    ...
    let target = match request.target_type.as_str() {
        "instance" => {
            let instance_id = Uuid::parse_str(&request.target_value)
                .map_err(|_| "Invalid instance ID")?;
            TunnelTarget::Instance(instance_id)
        }
        "port" => {
            let port: u16 = request.target_value.parse()
    ...
    let subdomain = match request.subdomain_type.as_str() {
        ...
        "custom" => {
            let subdomain_value = request.subdomain_value
                .ok_or("Custom subdomain requires a value")?;
            SubdomainConfig::Custom { subdomain: subdomain_value }
        }
    ...
    let tunnel = {
        let config_store = lock!(state.config_store)?;
        config_store.create_tunnel(request.name, server_id, target, subdomain, protocol, auto_start)?
    };
...
pub fn update_tunnel(
    ...
    let target = match (target_type.as_deref(), target_value) {
        ...
        (None, None) => None,
        _ => return Err("Invalid target: both target_type and target_value must be provided together".to_string()),
    };
    ...
    let tunnel = {
        let config_store = lock!(state.config_store)?;
        config_store.update_tunnel(tunnel_id, name, server_id_parsed, target, subdomain, protocol, None)?
    };
...
pub async fn start_tunnels(state: State<'_, AppState>) -> Result<(), String> {
    ...
    for tunnel in &mut updated_tunnels {
        if let SubdomainConfig::Random { generated: None } = &tunnel.subdomain {
            ...
            tunnel.subdomain = SubdomainConfig::Random { generated: Some(random_subdomain) };
            needs_save = true;
        }
    }
    ...
    frpc_manager.start(&updated_tunnels, &servers, &instances, admin_config.as_ref()).await
}
...
#[tauri::command]
pub async fn get_frpc_connection_status(state: State<'_, AppState>) -> Result<FrpcConnectionStatus, String> {
    // Find the frpc instance to get its port and credentials
    let (port, user, password) = {
        let config_store = lock!(state.config_store)?;
        let config = config_store.load()?;

        let frpc_instance = config.instances.iter()
            .find(|i| i.service_type == ServiceType::Frpc);

        match frpc_instance {
            Some(inst) => {
                let user = inst.config.get("admin_user")
                    .and_then(|v| v.as_str())
                    .unwrap_or("admin")
                    .to_string();
                let password = inst.config.get("admin_password")
                    .and_then(|v| v.as_str())
                    .unwrap_or("admin")
                    .to_string();
    ...
    for proxy_type in ["tcp", "udp", "http", "https", "stcp", "sudp", "xtcp"] {
        if let Some(arr) = json.get(proxy_type).and_then(|v| v.as_array()) {
            for proxy in arr {
                ...
                let status = proxy.get("status").and_then(|v| v.as_str()).unwrap_or("unknown").to_string();
                let local_addr = proxy.get("local_addr").and_then(|v| v.as_str()).unwrap_or("").to_string();
                let remote_addr = proxy.get("remote_addr").and_then(|v| v.as_str()).map(|s| s.to_string());
                let err = proxy.get("err").and_then(|v| v.as_str()).map(|s| s.to_string());
...
pub fn get_frpc_config() -> Result<String, String> {
    ...
    if !config_path.exists() {
        return Err("frpc configuration file does not exist. Configure a server and tunnels first.".to_string());
    }

    std::fs::read_to_string(&config_path)
        .map_err(|e| format!("Failed to read frpc config: {}", e))
}
